...
/**
 * main - Main Function displays the information contained in the
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	the_header = malloc(sizeof(Elf64_Ehdr));
	if (the_header == NULL)
	{
		close_elf(o);
		dprintf(STDERR_FILENO, "Error: Can't read file %s\n", argv[1]);
	...
	r = read(o, the_header, sizeof(Elf64_Ehdr));
	if (r == -1)
	{
		free(the_header);
		close_elf(o);
	...
	check_elf(the_header->e_ident);
	printf("ELF Header:\n");
	print_magic(the_header->e_ident);
	print_class(the_header->e_ident);
	print_data(the_header->e_ident);
	print_version(the_header->e_ident);
	print_osabi(the_header->e_ident);
	print_abi(the_header->e_ident);
	print_type(the_header->e_type, the_header->e_ident);
	print_entry(the_header->e_entry, the_header->e_ident);
